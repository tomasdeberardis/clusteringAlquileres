# ==================================================================================
#  PIPELINE FINAL Y DEFINITIVO (V18 - MODELO ROBUSTO CON DBSCAN Y LOGS COMPLETOS)
# ==================================================================================
import pandas as pd
import numpy as np
import warnings
...
from sklearn.cluster import DBSCAN # <-- IMPORTANTE: Cambiamos KMeans por DBSCAN
...
if not df_final.empty and len(df_final) > 10:
    ...
    # eps: La distancia máxima para que dos puntos sean vecinos. Es el parámetro más importante.
    # Un valor entre 0.5 y 1.5 es un buen punto de partida para datos normalizados.
    # min_samples: El número mínimo de puntos para formar una región densa.
    dbscan = DBSCAN(eps=0.75, min_samples=15)
    
    pipeline = Pipeline(steps=[('preprocessor', preprocessor), ('clusterer', dbscan)])
    
    print("  - [Entrenamiento] Entrenando el modelo DBSCAN para encontrar clústeres y aislar outliers...")
    pipeline.fit(X)
    df_final['Cluster'] = pipeline.named_steps['clusterer'].labels_
    
    # --- Reporte de Resultados de DBSCAN ---
    n_clusters = len(set(df_final['Cluster'])) - (1 if -1 in df_final['Cluster'] else 0)
    n_noise = list(df_final['Cluster']).count(-1)
    
    print(f"\n  - Modelo final entrenado.")
    print(f"  - DBSCAN encontró {n_clusters} clústeres distintos.")
    print(f"  - DBSCAN identificó {n_noise} propiedades como 'ruido' (outliers).")
    print("\n  - Cantidad de propiedades por clúster (el clúster -1 son los outliers):")
...
# --- PASO 5: INTERPRETACIÓN DETALLADA DE CLUSTERS (EXCLUYENDO OUTLIERS) ---
if 'Cluster' in df_final.columns:
    print("\n--- Iniciando Paso 5: Interpretación Detallada de Clusters ---")
    
    # Filtramos los outliers para el análisis y visualización de los clústeres
    ...
    if not df_clusters_only.empty:
        ...
        print("A continuación se muestra una descripción narrativa para cada clúster encontrado:")
        ...
        # El gráfico de dispersión ahora puede mostrar los outliers en un color diferente (ej. gris)
        sns.scatterplot(ax=axes[0, 0], data=df_final, x='Superficie_m2', y='Precio_USD_m2', hue='Cluster', palette='viridis', s=20, alpha=0.6)
        axes[0, 0].set_title('Visión General (Clúster -1 = Outliers)')
        
        # Los boxplots se hacen solo con los datos clusterizados para no distorsionar las escalas
        ...
        print("  - No se encontraron clústeres suficientemente densos. La mayoría de los datos fue clasificado como ruido.")
...
if 'Cluster' in df_final.columns and not df_clusters_only.empty:
    ...
    # La valuación se hace solo para las propiedades que cayeron en un clúster
    ...
    # La valuación se define como 'N/A' para los outliers (clúster -1)
    ...
    # El resto del código de logs y exportación se mantiene igual
    columnas_export = ['ID', 'URL', 'Fecha', 'Barrio', 'Ambientes', 'Superficie_m2', 'Precio_USD', 'Precio_USD_m2', 'Cluster', 'Precio_Promedio_Cluster_m2', 'Valuacion', 'Indice_Valor_Barrio']
    ...
    print(f"\n- Archivo '{output_filename}' generado. Incluye una columna 'Cluster' donde -1 indica un outlier.")
